...
namespace googlecloud1
{
    ...
    class FileDownload : Download
    {
        ...
        protected override async void StreamThread()
        {
            ...
            this.file = File.Create(FilePath.GetPath(this.filename));
            MemoryStream mstream = new MemoryStream();
            byte[] buf = new byte[this.StreamBlockSize];
            while((ReadSize = stream.Read(buf, 0, this.StreamBlockSize)) > 0)
            {
        ...
        private void SaveToFile(string filename, byte[] Data, int Size)
        {
            try
            {
